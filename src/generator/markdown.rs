...
pub fn generate(
    ...
    out.push_str("> Generated by [starmap](https://github.com/nozomi-koborinai/starmap).\n\n");
...
/// Convert a section name to the anchor GitHub generates for that heading.
///
/// GitHub's heading slugger lowercases the text, drops characters that are
/// neither alphanumeric, `-`, nor `_`, then replaces each space with `-`
/// WITHOUT trimming or collapsing runs. Two quirks matter for our
/// emoji-prefixed list names, both verified against GitHub's rendered anchors:
///   * The emoji itself is dropped, but the space that followed it still
///     becomes a leading `-` (e.g. `🤖 AI Frameworks` -> `-ai-frameworks`).
///   * A variation selector (U+FE0F) or zero-width joiner (U+200D) survives,
///     so `🛠️ Dev Tools` -> `\u{FE0F}-dev-tools`.
///
/// Replicating this exactly is what keeps the Contents links clickable; the
/// previous `split_whitespace().join("-")` dropped the leading/duplicate `-`
/// and broke every emoji-prefixed link.
fn to_anchor(name: &str) -> String {
    name.to_lowercase()
        ...
        .filter_map(|c| match c {
            ' ' => Some('-'),
            '-' | '_' => Some(c),
            // Variation selectors and ZWJ are preserved by GitHub's slugger.
            '\u{200D}' | '\u{FE00}'..='\u{FE0F}' => Some(c),
...
mod tests {
    ...
    fn test_generate_basic() {
        ...
        assert!(
            md.contains("> Generated by [starmap](https://github.com/nozomi-koborinai/starmap).")
        );
    ...
    #[test]
    fn to_anchor_plain_ascii_heading() {
        assert_eq!(to_anchor("Uncategorized"), "uncategorized");
        assert_eq!(to_anchor("Dev Tools"), "dev-tools");
    }

    #[test]
    fn to_anchor_matches_github_slugger_for_emoji_headings() {
        // Ground truth captured from GitHub's rendered <h2> anchors on
        // koborin-ai/stars. Emoji base chars are dropped; the space that
        // followed becomes a leading '-'; '&' removal leaves a doubled '-';
        // a U+FE0F variation selector survives as a leading invisible char.
        assert_eq!(to_anchor("🤖 AI Frameworks"), "-ai-frameworks");
        assert_eq!(to_anchor("🦾 Agent Tools"), "-agent-tools");
        assert_eq!(to_anchor("⚡ Low-level & Perf"), "-low-level--perf");
        // ☁️ = U+2601 U+FE0F, 🛠️ = U+1F6E0 U+FE0F, 🗄️ = U+1F5C4 U+FE0F
        assert_eq!(
            to_anchor("\u{2601}\u{FE0F} Cloud & DevOps"),
            "\u{FE0F}-cloud--devops"
        );
        assert_eq!(
            to_anchor("\u{1F6E0}\u{FE0F} Dev Tools"),
            "\u{FE0F}-dev-tools"
        );
        assert_eq!(
            to_anchor("\u{1F5C4}\u{FE0F} Databases"),
            "\u{FE0F}-databases"
        );
    }
